        ...
        switch (med.position) {
            ...
            return (
              ...
              dayItem.day() !== 6 && dayItem.day() !== 0
                ? // exm: до еды за 45 минут
                  halfHourItem.isSame(
                    firstMealWeekdays.subtract(med.interval, 'minute'),
                    'hour',
                  ) &&
                ...
                : // weekend
                  // exm: до еды за 45 минут
                  halfHourItem.isSame(
                    firstMealWeekend.subtract(med.interval, 'minute'),
                    'hour',
                  ) &&
          ...
          case 'after': // после
            return (
              // weekday
              dayItem.day() !== 6 && dayItem.day() !== 0
                ? // exm: спустя 60 минут после еды
                  halfHourItem.isSame(
                    firstMealWeekdays.add(med.interval, 'minute'),
                    'hour',
                  ) &&
                    firstMealWeekdays.minute() - halfHourItem.minute() >= 0 &&
                    firstMealWeekdays.minute() - halfHourItem.minute() < 30 && (
                ...
                : // weekend
                  // exm: спустя 60 минут после еды
                  halfHourItem.isSame(
                    firstMealWeekend.add(med.interval, 'minute'),
                    'hour',
                  ) &&
                    firstMealWeekend.minute() - halfHourItem.minute() >= 0 &&
                    firstMealWeekend.minute() - halfHourItem.minute() < 30 && (
          ...
          (halfHourItem.isSame(firstMealWeekdays, 'hour') &&
            ...
            [...new Array(med.quantity)].map((_, index) =>
              ...
              firstMealWeekend.clone().add(betweenMealsWeekend, 'm').minute() -
                halfHourItem.minute() <
                30 ? (
                <RiMedicineBottleLine key={index} style={{ color: 'red' }} />
              ) : null,
            )
        : // weekend
          (halfHourItem.isSame(firstMealWeekend, 'hour') &&
